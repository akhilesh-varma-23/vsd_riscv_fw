...
- GPIOx – GPIOx - Pointer to the GPIO peripheral (GPIOA, GPIOC, GPIOD, etc.)
- pin – Pin number (0–15)
...
None
### Notes
- Sets up SysTick without enabling interrupts. For interrupt based delays, ensure the NVIC exception is enabled.
### Example
```
HAL_Delay_Init();
```
...
None
### Notes
- Uses a simple busy-wait loop. Accuracy depends on CPU clock.
### Example
```
HAL_Delay_ms(500);
```
...
None
### Notes
- Uses a busy-wait loop based on CPU cycles. Approximate delay.
### Example
```
HAL_Delay_us(50000);
```
